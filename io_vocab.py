...
def add_word_to_vocab(word, words, vocab_map, vocab_size):
    ...
    Especially useful for handling <unk>.
...
def update_vocab(words, vocab_map, freq_map, freq, max_vocab_size):
    ...
    new_words = [unk]
    new_vocab_map = {unk: VocabConstants.UNK_INDEX}
    new_freq_map = {unk: 0}
    vocab_size = 1
    if freq > 0:
        for word in words:
...
def get_mapped_words(sentences, words_map_file):
    words=[]
    for line in sentences:
        ids = line.strip().split()
        for i in xrange(len(ids)-1):
            words.append([ids[i],ids[i+1]])
    write_mapped_words(words, words_map_file)
    return words   
...
def to_id_int(tokens, vocab_map, offset=0):
    ...
    """
    unk = VocabConstants.UNK
    return [int(vocab_map[token] + offset) if token in vocab_map else int(vocab_map[unk] + offset) for token in tokens]
...
def write_mapped_words(words, words_map_file):
    f = open(words_map_file, 'w')
    for word in words:
